...
You can also force a dirty state:
  ...
  user.name_changed? # => false
  user.save # does nothing as no attributes are dirty
...
This will load all user documents in your database sorted by created_at.
  ...
  User.all :key => (Time.now- 10)..(Time.now), :descending => true
...
If you have larger structures and you only want to load some attributes you can customize the view:
  ...
  class User
    ...
    view :all, :key => :created_at, :properties => [:name]
  end
  
  User.everyone.first.name # => "joe"
  User.everyone.first.bio # => nil
  
You can also pass in custom map/reduce functions and options:

  class User
    view :all, :map => "function(doc) { emit(doc.created_at, null)}", :include_docs => true
  end
...
To make testing easier and faster you can replace parts of the Couch Potato infrastructure with stubs/mocks:
  ...
  describe User, 'save' do
    it 'should save' do
      persister = stub('persister')
      user = User.new
      user.persister = persister
      persister.should_receive(:save_document)
      user.save
    end
  end
  
For more information look at the Persister class and the specs (namely spec/unit/create_spec.rb).
