...
describe CouchPotato::View::ViewQuery, 'query_view!' do
  ...
  it "should not update a view when there is a list function but no list function is passed" do
    ...
    db.should_not_receive(:save_doc)
    CouchPotato::View::ViewQuery.new(db, 'design', {:view4 => {:map => '<map_code>', :reduce => '<reduce_code>'}}, {}).query_view!
  end
  
  it "should query CouchRest directly when querying a list" do
    db = stub('db').as_null_object
    CouchRest.should_receive(:get).with('http://127.0.0.1:5984/couch_potato_test/_design/my_design/_list/list1/view5?key=1')
    CouchPotato::View::ViewQuery.new(db, 'my_design', {:view5 => {:map => '<map_code>', :reduce => '<reduce_code>'}}, :list1 => '<new_list_code>').query_view!(:key => 1)
  end
